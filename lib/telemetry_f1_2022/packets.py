...
class PacketHeader(Packet):
    _fields_ = [
        ...
        ("m_packet_format", ctypes.c_uint16),    #  2022
...
class Penalty_v2022_1_9(Packet):
    _fields_ = [
        ("m_penalty_type", ctypes.c_uint8),    #  Penalty type – see Appendices
        ("m_infringement_type", ctypes.c_uint8),    #  Infringement type – see Appendices
        ("m_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the car the penalty is applied to
...
class SpeedTrap_v2022_1_12(Packet):
    _fields_ = [
        ("m_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the vehicle triggering speed trap
        ("m_speed", ctypes.c_float),    #  Top speed achieved in kilometres per hour
        ("m_is_overall_fastest_in_session", ctypes.c_uint8),    #  Overall fastest speed in session = 1, otherwise 0
...
class EventDataDetails_v2022_1_9(ctypes.Union, PacketMixin):
    _fields_ = [
        ("m_fastest_lap", FastestLap),
        ("m_retirement", Retirement),
        ("m_team_mate_in_pits", TeamMateInPits),
        ("m_race_winner", RaceWinner),
        ("m_penalty", Penalty_v2022_1_9),
        ("m_speed_trap", SpeedTrap),
        ("m_start_lights", StartLIghts),
        ("m_drive_through_penalty_served", DriveThroughPenaltyServed),
        ("m_stop_go_penalty_served", StopGoPenaltyServed),
...
class EventDataDetails_v2022_1_12(ctypes.Union, PacketMixin):
    _fields_ = [
        ("m_fastest_lap", FastestLap),
        ("m_retirement", Retirement),
        ("m_team_mate_in_pits", TeamMateInPits),
        ("m_race_winner", RaceWinner),
        ("m_penalty", Penalty_v2022_1_9),
        ("m_speed_trap", SpeedTrap_v2022_1_12),
        ("m_start_lights", StartLIghts),
        ("m_drive_through_penalty_served", DriveThroughPenaltyServed),
        ("m_stop_go_penalty_served", StopGoPenaltyServed),
        ("m_flashback", Flashback),
...
class PacketEventData_v2022_1_9(Packet):
    _fields_ = [

        ("m_header", PacketHeader),    #  Header

        ("m_event_string_code", ctypes.c_uint8 * 4),    #  Event string code, see below
        ("m_event_details", EventDataDetails_v2022_1_9),    #  Event details - should be interpreted differently
        #  for each type

    ]

class PacketEventData_v2022_1_12(Packet):
    _fields_ = [

        ("m_header", PacketHeader),    #  Header

        ("m_event_string_code", ctypes.c_uint8 * 4),    #  Event string code, see below
        ("m_event_details", EventDataDetails_v2022_1_12),    #  Event details - should be interpreted differently
        #  for each type
...
class PacketLapData_v2022_1_8(Packet):
    _fields_ = [
...
HEADER_FIELD_TO_PACKET_TYPE_OVERRIDE = {
    (2022, 1, 2, 1, 8): PacketLapData_v2022_1_8,
    (2022, 1, 3, 1, 9): PacketEventData_v2022_1_9,
    (2022, 1, 3, 1, 12): PacketEventData_v2022_1_12
}
