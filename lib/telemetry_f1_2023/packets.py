...
https://answers.ea.com/t5/General-Discussion/F1-23-UDP-Specification/m-p/12632888
"""
...
class Packet(ctypes.LittleEndianStructure, PacketMixin):
    """The base packet class for API version 2023"""
...
class PacketHeader(Packet):
    _fields_ = [
        ...
        ("m_packet_format", ctypes.c_uint16),    #  2023
        ("m_game_year", ctypes.c_uint8),    #  Game year - last two digits e.g. 23
        ...
        ("m_overall_frame_identifier", ctypes.c_uint32),    #  Overall identifier for the frame the data was retrieved
...
class PacketTestData(PacketHeader):
    _fields_ = [
...
class CarMotionData(Packet):
    _fields_ = [
        ...
        ("m_world_position_x", ctypes.c_float),    #  World space X position - metres
        ("m_world_position_y", ctypes.c_float),    #  World space Y position
        ("m_world_position_z", ctypes.c_float),    #  World space Z position
        ("m_world_velocity_x", ctypes.c_float),    #  Velocity in world space X – metres/s
...
class Penalty(Packet):
    _fields_ = [
        ...
        ("m_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the car the penalty is applied to
        ("m_other_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the other car involved
        ("m_time", ctypes.c_uint8),    #  Time gained, or time spent doing action in seconds
...
class SpeedTrap(Packet):
    _fields_ = [
        ("m_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the vehicle triggering speed trap
        ("m_speed", ctypes.c_float),    #  Top speed achieved in kilometres per hour
        ("m_is_overall_fastest_in_session", ctypes.c_uint8),    #  Overall fastest speed in session = 1, otherwise 0
        ("m_is_driver_fastest_in_session", ctypes.c_uint8),    #  Fastest speed for driver in session = 1, otherwise 0
        ("m_fastest_vehicle_idx_in_session", ctypes.c_uint8),    #  Vehicle index of the vehicle that is the fastest
        #  in this session
        ("m_fastest_speed_in_session", ctypes.c_float),    #  Speed of the vehicle that is the fastest
        #  in this session
    ]
...
class DriveThroughPenaltyServed(Packet):
    _fields_ = [
        ("m_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the vehicle serving drive through
    ]

class StopGoPenaltyServed(Packet):
    _fields_ = [
        ("m_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the vehicle serving stop go
    ]

class Flashback(Packet):
    _fields_ = [
        ("m_flashback_frame_identifier", ctypes.c_uint32),    #  Frame identifier flashed back to
...
class Overtake(Packet):
    _fields_ = [
        ("m_overtaking_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the vehicle overtaking
        ("m_being_overtaken_vehicle_idx", ctypes.c_uint8),    #  Vehicle index of the vehicle being overtaken
    ]
...
class LapData(Packet):
    _fields_ = [
        ...
        ("m_sector1_time_minutes", ctypes.c_uint8),    #  Sector 1 whole minute part
        ("m_sector2_time_in_ms", ctypes.c_uint16),    #  Sector 2 time in milliseconds
        ("m_sector2_time_minutes", ctypes.c_uint8),    #  Sector 2 whole minute part
        ("m_delta_to_car_in_front_in_ms", ctypes.c_uint16),    #  Time delta to car in front in milliseconds
        ("m_delta_to_race_leader_in_ms", ctypes.c_uint16),    #  Time delta to race leader in milliseconds
...
class LapHistoryData(Packet):
    _fields_ = [
        ...
        ("m_sector1_time_minutes", ctypes.c_uint8),    #  Sector 1 whole minute part
        ("m_sector2_time_in_ms", ctypes.c_uint16),    #  Sector 2 time in milliseconds
        ("m_sector1_time_minutes", ctypes.c_uint8),    #  Sector 2 whole minute part
        ("m_sector3_time_in_ms", ctypes.c_uint16),    #  Sector 3 time in milliseconds
        ("m_sector3_time_minutes", ctypes.c_uint8),    #  Sector 3 whole minute part
...
class PacketMotionExData(Packet):
    _fields_ = [
        ...
        ("m_height_of_c_o_g_above_ground", ctypes.c_float),    #  Height of centre of gravity above ground    
...
class PacketLapData(Packet):
    _fields_ = [
        ...
        ("m_time_trial_p_b_car_idx", ctypes.c_uint8),    #  Index of Personal Best car in time trial (255 if invalid)
...
class PacketSessionData(Packet):
    _fields_ = [
        ...
        ("m_session_length", ctypes.c_uint8),    #  0 = None, 2 = Very Short, 3 = Short, 4 = Medium
...
class TyreSetData(Packet):
    _fields_ = [
        ...
        ("m_recommended_session", ctypes.c_uint8),    #  Recommended session for tyre set
...
HEADER_FIELD_TO_PACKET_TYPE_OVERRIDE = {
}
